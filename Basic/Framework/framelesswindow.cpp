...

{
    ...
    m_minWidth = 50;
    m_minHeight = 50;
    connect(this, &QQuickWindow::widthChanged, this, [=](){
...
{
    //根据实际情况修改 showMaximized()后 全屏的 windowState() 是否等于 Qt::WindowMaximized
...
void FramelessWindow::setWindowGeometry(const QPoint &pos)
{
    ...
    static auto set_geometry_func = [this](const QSize &size, const QPoint &pos) {
        ...
        if (size.width() > m_minWidth) {
            ...
            if (pos.x() != temp_pos.x())
                temp_pos.setX(m_oldPos.x() +  m_oldSize.width() - m_minWidth);
        }
        if (size.height() > m_minHeight) {
            ...
            if (pos.y() != temp_pos.y())
                temp_pos.setY(m_oldPos.y() + m_oldSize.height() - m_minHeight);
        }

        if (temp_size.width() <= m_minWidth){
            temp_size.setWidth(m_minWidth);
        }
        if (temp_size.height() <= m_minHeight){
            temp_size.setHeight(m_minHeight);
        }
